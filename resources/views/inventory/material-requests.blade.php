...
<form method="post" action="/admin/inventory/material-requests/{{ $mr->id }}/approve" class="mt-2 inline">@csrf<button data-confirm="Setujui permintaan material ini?" class="rounded-lg bg-emerald-700 px-4 py-2 text-xs font-bold text-white">Approve</button></form>
@elseif($mr->status === 'approved')
<form method="post" action="/admin/inventory/material-requests/{{ $mr->id }}/issue" class="mt-2 inline">@csrf<button data-confirm="Terbitkan seluruh sisa material dari gudang dan posting jurnal biaya proyek?" class="rounded-lg bg-[var(--brand-primary)] px-4 py-2 text-xs font-bold text-white">Issue ke proyek</button></form>
@endif
@endif
</x-ui.card>

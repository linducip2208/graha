<x-layouts.app title="Project & Bored Pile"><section class="mx-auto max-w-7xl px-6 py-10"><h1 class="text-3xl font-black">Project & Bored Pile</h1>@if(session('status'))<div class="mt-4 rounded-xl bg-emerald-50 p-4">{{ session('status') }}</div>@endif @if($errors->any())<div class="mt-4 rounded-xl bg-red-50 p-4 text-red-700">{{ $errors->first() }}</div>@endif
...
@if($projects->count() > 1)<form method="get" class="no-print"><select name="project" onchange="this.form.submit()" class="rounded-xl border p-2 text-sm">@foreach($projects as $p)<option value="{{ $p->id }}" @selected($schedule['project']->id === $p->id)>{{ $p->code }} — {{ $p->name }}</option>@endforeach</select></form>@endif
</div>

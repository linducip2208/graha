<x-layouts.app title="Foundation Control â€” {{ $project->code }}">
<div class="page-container">
<x-ui.page-header docs="/docs/bored-pile/foundation-control-tower" title="Foundation Control Tower" subtitle="{{ $project->code }} â€” {{ $project->name }}" status="{{ str($project->status)->replace('_',' ') }}">
...
<div class="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
    ...
    <x-ui.stat-card label="Beton Disetujui Hari Ini" value="{{ number_format($kpi['concrete_today'], 2) }} mÂ³" />
...
{{-- Advanced KPI (ADR-076/077): klik KPI â†’ filter tabel pile --}}
    ...
    <div class="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <tbody>@forelse(${$laKey} as $row)
            <tr class="border-b">
                ...
                <td class="text-right font-mono">{{ $row['blockers'] }}{{ $row['active_constraints'] > 0 ? ' Â· âš '.$row['active_constraints'] : '' }}</td>
            </tr>
...
@if($filter)
<p class="mt-6 inline-block rounded-xl bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700">Filter aktif: {{ str($filter)->replace('_', ' ') }} Â· {{ $rows->count() }} pile <a href="{{ route('projects.foundation-control', $project) }}" class="ml-2 underline">hapus filter</a></p>
@endif

{{-- Risiko --}}
<h2 class="mt-8 font-black">Risk Radar</h2>
<p class="text-xs text-slate-400">Deterministik dari data nyata â€” tanpa AI. WATCH â‰¥ 15 poin, CRITICAL bila ada temuan kritis atau skor â‰¥ 60.</p>
<div class="mt-2 flex flex-wrap gap-3">
    <span class="rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-1.5 text-sm font-bold text-emerald-800">HEALTHY Â· {{ $riskCounts['healthy'] }}</span>
    <span class="rounded-xl border border-amber-300 bg-amber-50 px-3 py-1.5 text-sm font-bold text-amber-800">WATCH Â· {{ $riskCounts['watch'] }}</span>
    <span class="rounded-xl border border-red-300 bg-red-50 px-3 py-1.5 text-sm font-bold text-red-700">CRITICAL Â· {{ $riskCounts['critical'] }}</span>
</div>
...
<table class="w-full text-xs">
    ...
    @foreach($rows as $row)
        ...
        <tr class="border-b hover:bg-indigo-50/40">
            ...
            <td class="p-2">{{ $row['status_label'] }}@if($row['pile']->acceptance) Â· {{ $row['pile']->acceptance->status }}@endif</td>
            ...
            <td class="p-2"><a href="{{ route('piles.passport', $row['pile']) }}" class="font-bold text-indigo-600 hover:underline">Passport â†’</a></td>
        </tr>
        @endif
    @endforeach
...
<div class="relative mt-2 h-96 rounded-2xl border bg-gradient-to-br from-slate-50 to-stone-100 shadow-inner dark:from-[#0d1728] dark:to-[#101c30]">
    ...
    <a href="{{ route('piles.passport', $point['pile']) }}" title="{{ $point['pile']->pile_number }} â€” {{ strtoupper($point['level']) }}"
...
<div class="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
    ...
    <div class="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div class="mt-2 flex flex-wrap gap-1.5">
            ...
            <a href="{{ route('piles.passport', $item['pile']) }}" title="{{ $item['pile']->pile_number }} Â· {{ $item['pile']->status }} Â· {{ strtoupper($item['level']) }}" class="min-h-[44px] min-w-[52px] rounded-lg border bg-emerald-50 border-emerald-200 px-1.5 py-1 text-center text-[10px] font-bold leading-tight{{ $item['level'] !== 'healthy' ? ' ring-2 ring-offset-1 ' . ($item['level'] === 'critical' ? 'ring-red-500' : 'ring-amber-400') : '' }}">{{ $item['pile']->pile_number }}<br><span class="font-normal text-slate-400">{{ $item['status_short'] }}</span></a>
            @elseif($item['pile']->status === 'hold')
            <a href="{{ route('piles.passport', $item['pile']) }}" title="{{ $item['pile']->pile_number }} Â· hold Â· {{ strtoupper($item['level']) }}" class="min-h-[44px] min-w-[52px] rounded-lg border bg-red-100 border-red-300 px-1.5 py-1 text-center text-[10px] font-bold leading-tight{{ $item['level'] !== 'healthy' ? ' ring-2 ring-offset-1 ' . ($item['level'] === 'critical' ? 'ring-red-500' : 'ring-amber-400') : '' }}">{{ $item['pile']->pile_number }}<br><span class="font-normal text-red-500">HOLD</span></a>
            @else
            <a href="{{ route('piles.passport', $item['pile']) }}" title="{{ $item['pile']->pile_number }} Â· {{ $item['pile']->status }} Â· {{ strtoupper($item['level']) }}" class="min-h-[44px] min-w-[52px] rounded-lg border bg-slate-50 border-slate-200 px-1.5 py-1 text-center text-[10px] font-bold leading-tight{{ $item['level'] !== 'healthy' ? ' ring-2 ring-offset-1 ' . ($item['level'] === 'critical' ? 'ring-red-500' : 'ring-amber-400') : '' }}">{{ $item['pile']->pile_number }}<br><span class="font-normal text-slate-400">{{ $item['status_short'] }}</span></a>
            @endif
            @endforeach
        </div>
    </div>
...
<div class="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
    ...
    <x-ui.stat-card label="Beton Disetujui" value="{{ number_format($kpi['concrete_today'], 2) }} mÂ³" />

<x-layouts.public title="Dokumentasi" description="Cara kerja ERP Graha Pondasi: akun demo, struktur menu, tutorial 7 fase alur bisnis kontraktor pondasi, fitur, pajak, dan QMS.">
<div class="mx-auto grid max-w-6xl gap-10 px-5 py-14 lg:grid-cols-[240px_1fr]">
{{-- ===== SIDEBAR TOC ===== --}}
<aside class="no-print lg:sticky lg:top-24 lg:self-start">
<p class="text-xs font-extrabold uppercase tracking-widest text-slate-400">Dokumentasi</p>
<nav aria-label="Daftar isi" class="mt-4 space-y-1 text-sm">
@foreach([['#akun','Akun Demo'],['#menu','Struktur Menu'],['#tutorial','Tutorial 7 Fase'],['#fitur','Fitur Utama'],['#pajak','Pajak dalam Sistem'],['#verify','Verifikasi Tanda Tangan'],['#qms','QMS & Keamanan Data']] as [$href,$label])
<a href="{{ $href }}" class="block rounded-lg px-3 py-2 font-semibold text-slate-600 transition hover:bg-slate-100 hover:text-sky-700">{{ $label }}</a>
@endforeach
</nav>
<div class="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-4">
<p class="text-sm font-bold">Siap mencoba?</p>
<p class="mt-1 text-xs leading-relaxed text-slate-500">Masuk dengan akun demo dan jelajahi seluruh modul sesuai kewenangan.</p>
<a href="/login" class="mt-3 inline-block rounded-xl bg-sky-700 px-4 py-2 text-xs font-bold text-white hover:bg-sky-800">Masuk ke Sistem</a>
</div>
</aside>
...
<h1 class="mt-1 text-3xl font-black tracking-tight sm:text-4xl">Cara Kerja ERP Graha Pondasi</h1>
<p class="mt-4 max-w-3xl leading-relaxed text-slate-600">Sistem mendukung implementasi dan bukti penerapan sistem manajemen mutu; tidak menyatakan sertifikasi ISO. Ikuti tutorial berurutan sesuai alur bisnis kontraktor pondasi: setup → master data → pengadaan → pelaksanaan → tagihan & pajak → laporan.</p>

<h2 id="akun" class="mt-14 scroll-mt-28 text-2xl font-black">Akun Demo</h2>
<div class="mt-4 overflow-x-auto rounded-2xl border border-slate-200 bg-white"><table class="w-full text-sm"><thead><tr><th>Role</th><th>Email</th><th>Password</th><th>Cakupan</th></tr></thead><tbody>
@foreach([
...
<h2 id="menu" class="mt-14 scroll-mt-28 text-2xl font-black">Struktur Menu</h2>
<p class="mt-2 text-slate-500">Menu tampil adaptif mengikuti permission membership perusahaan Anda — sidebar ringkas, workspace aktif membuka submenu kontekstual, dan semua capability tetap terjangkau via launcher aplikasi.</p>
...
<article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p class="text-xs font-bold uppercase tracking-widest text-slate-400">{{ $group['label'] }}</p><ul class="mt-2 space-y-1.5 text-sm text-slate-700">@foreach($group['items'] as $item)<li>{{ $item['label'] }}</li>@endforeach</ul>@if($childLabels)<p class="mt-2 text-xs leading-relaxed text-slate-400">{{ $childLabels }}</p>@endif</article>
@endforeach
</div>

<h2 id="tutorial" class="mt-14 scroll-mt-28 text-2xl font-black">Tutorial 7 Fase — Sesuai Alur Bisnis</h2>
...
<article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h3 class="font-black">{{ $faseTitle }}</h3><ol class="mt-3 list-decimal space-y-1.5 pl-6 text-sm leading-relaxed text-slate-700">@foreach($steps as $step)<li>{!! $step !!}</li>@endforeach</ol></article>
@endforeach
</div>

<h2 id="fitur" class="mt-14 scroll-mt-28 text-2xl font-black">Fitur Utama</h2>
...
<article class="card-lift rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 class="font-bold">{{ $title }}</h3><p class="mt-2 text-sm leading-relaxed text-slate-500">{{ $desc }}</p></article>
@endforeach
</div>

<h2 id="pajak" class="mt-14 scroll-mt-28 text-2xl font-black">Pajak dalam Sistem</h2>
<div class="mt-4 overflow-x-auto rounded-2xl border border-slate-200 bg-white"><table class="w-full text-sm"><thead><tr><th>Jenis</th><th>Muncul Saat</th><th>Jurnal</th></tr></thead><tbody>
...
<h2 id="verify" class="mt-14 scroll-mt-28 text-2xl font-black">Verifikasi Tanda Tangan Digital</h2>
<p class="mt-2 leading-relaxed text-slate-600">Setiap dokumen bertanda tangan memiliki tautan verifikasi QR. Halaman verifikasi menampilkan status integritas dokumen, versi, penandatangan, dan waktu penandatanganan — tanpa membuka data internal lainnya.</p>

<h2 id="qms" class="mt-14 scroll-mt-28 text-2xl font-black">QMS &amp; Keamanan Data</h2>

...
class AccountingService
{
    ...
    public function post(int $companyId, string $date, string $sourceType, string $sourceId, string $description, array $lines, string $key, User $actor): Journal
    {
        return DB::transaction(function () use ($companyId, $date, $sourceType, $sourceId, $description, $lines, $key, $actor) {
            ...
            $journal = Journal::create(['company_id' => $companyId, 'fiscal_period_id' => $period->id, 'number' => $this->numbers->next($companyId, 'journal'), 'journal_date' => $date, 'source_type' => $sourceType, 'source_id' => $sourceId, 'description' => $description, 'status' => 'draft', 'idempotency_key' => $key, 'created_by' => $actor->id]);
            foreach ($lines as $line) {

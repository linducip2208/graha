...
class ProgressBillingService
{
    ...
    public function create(Project $project, array $data, User $actor): ProgressBilling
    {
        return DB::transaction(function () use ($project, $data, $actor) {
            ...
            $net = bcsub(bcsub($gross, $retention, 2), $advance, 2);
            ...
            }$billing = ProgressBilling::create([...$data, 'company_id' => $project->company_id, 'project_id' => $project->id, 'retention_amount' => $retention, 'net_receivable' => $net, 'created_by' => $actor->id]);

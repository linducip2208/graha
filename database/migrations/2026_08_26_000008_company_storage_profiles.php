...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasTable('company_storage_profiles')) {
            Schema::create('company_storage_profiles', function (Blueprint $table) {
                ...
                $table->index(['company_id', 'is_active', 'is_default_evidence'], 'csp_evidence_default_idx');
                $table->index(['company_id', 'is_active', 'is_default_document'], 'csp_document_default_idx');
            });
        } else {
            Schema::table('company_storage_profiles', function (Blueprint $table) {
                if (! Schema::hasIndex('company_storage_profiles', 'csp_evidence_default_idx')) {
                    $table->index(['company_id', 'is_active', 'is_default_evidence'], 'csp_evidence_default_idx');
                }
                if (! Schema::hasIndex('company_storage_profiles', 'csp_document_default_idx')) {
                    $table->index(['company_id', 'is_active', 'is_default_document'], 'csp_document_default_idx');
                }
            });
        }

        if (! Schema::hasColumn('stored_files', 'storage_profile_id')) {
        ...
        if (! Schema::hasColumn('stored_files', 'storage_locator')) {
            Schema::table('stored_files', function (Blueprint $table) {
                $table->json('storage_locator')->nullable()->after('object_key');
            });
        }

        if (! Schema::hasColumn('document_versions', 'storage_profile_id')) {
            Schema::table('document_versions', function (Blueprint $table) {
                $table->foreignId('storage_profile_id')->nullable()->constrained('company_storage_profiles')->nullOnDelete();
            });
        }
        if (! Schema::hasColumn('document_versions', 'storage_locator')) {
            Schema::table('document_versions', function (Blueprint $table) {

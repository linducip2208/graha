...
- ADR-048 Accepted: keputusan Bid/No-Bid memakai scoring faktor data nyata (margin, cover HPS, kompetisi, termin) dengan bobot/ambang configurable; faktor tanpa data tidak dikarang dan memaksa hasil Perlu Review — bukan pengganti keputusan manusia.
- ADR-049 Accepted: constraint log proyek memakai transisi open → in_progress → resolved yang terjaga; resolusi wajib catatan dan kendala selesai tidak dibuka ulang (buat entri baru).
...
- ADR-051 Accepted: EVM ringkas (CPI/SPI) dihitung saat baca dari EV progres terkontrak, PV jadwal, dan AC project cost ledger; hanya tampil bila AC>0 — tanpa input manual baru.

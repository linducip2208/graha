...
- ADR-076 Accepted: App Launcher adalah kartu per WORKSPACE yang dibangun dari effective navigation (Navigation::groups: permission x edition x nav composer x terminology); config/app-launcher.php hanya metadata presentasi (cover/deskripsi) dan TIDAK pernah menjadi sumber href/permission. Stable key per grup wajib ada; fallback cover: company custom (disk privat, dioptimalkan WebP 1200x675 via GD) -> registry default -> gradient tema + ikon. Mode tampilan (visual/compact/list) disimpan di localStorage per user; preferensi default per company via Experience Studio (launcher_config).
